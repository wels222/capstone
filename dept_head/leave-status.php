  ...
  <body>
          ...
          <section id="leave-status-content" class="content-section active">
            ...
            <section id="analytics" class="mt-6">
              ...
              <div class="mt-4 space-y-4">
                ...
                <!-- Employee Analysis (Department-specific) -->
                <div class="bg-white rounded-xl shadow p-4">
                  <div class="flex items-center justify-between mb-2">
                    <h4 class="font-semibold text-gray-800">
                      Team Member Leave Distribution
                    </h4>
                    <span class="text-xs text-gray-500"
                      >By employee in your department</span
                    >
                  </div>
                  <div class="h-80">
                    <canvas
                      id="chart-employee"
                      aria-label="Leave by employee"
                    ></canvas>
                  </div>
                  <div
                    class="mt-3 p-3 bg-indigo-50 rounded-lg border-l-4 border-indigo-500"
                  >
                    <p class="text-xs text-gray-700" id="interpret-employee">
                      Loading interpretation...
                    </p>
                  </div>
                </div>
    ...
    <script>
      ...
      document.addEventListener("DOMContentLoaded", () => {
        ...
        let monthLeaves = [];
        let analyticsData = null; // Enhanced analytics data from dept_head_leave_analytics.php
        let isLoading = false;
        ...
        // Get current user's department to filter results
        (async function getCurrentDept() {
          try {
            const resp = await fetch("../api/current_user.php");
        ...
        async function fetchLeavesForMonth(date) {
          ...
          try {
            ...
            // Filter to current department for Dept Head view (client-side)
            if (currentDept)
        ...
        // Old fetchAnalyticsForMonth removed - now using fetchEnhancedAnalytics instead
        ...
        // Fetch enhanced analytics from dept head API
        async function fetchEnhancedAnalytics(date) {
          const y = date.getFullYear();
          const m = date.getMonth() + 1;
          try {
            console.log(`Fetching dept analytics for ${y}-${m}...`);
            const res = await fetch(
              `../api/dept_head_leave_analytics.php?month=${m}&year=${y}`
            );
            if (!res.ok) {
        ...
        function updateDailyTrendChart() {
          ...
          if (interpretEl) {
            ...
            if (totalDays === 0 || maxCount === 0) {
              interpretation =
                "No leave activity for your department this period.";
            } else if (maxCount === minCount) {
              interpretation = `Consistent pattern: ${maxCount} team member${
                maxCount !== 1 ? "s" : ""
              } on leave daily. Stable coverage maintained.`;
            } else {
              const variance = maxCount - minCount;
              interpretation = `Your department: ${minCount} to ${maxCount} on leave (avg: ${avgCount.toFixed(
                1
              )}). `;
              if (variance > avgCount) {
                interpretation += `Significant fluctuation - coordinate workload during high-leave days.`;
              } else {
                interpretation += `Moderate variation - staffing levels manageable.`;
              }
            }
        ...
        function updateEmployeeChart() {
          if (!analyticsData) return;
          const ctx = document.getElementById("chart-employee");
          if (!ctx) return;

          const { by_employee, interpretations } = analyticsData;
          const employees = by_employee ? Object.keys(by_employee) : [];

          if (employees.length === 0) {
            employees.push("No data");
            by_employee["No data"] = { approved: 0, pending: 0, declined: 0 };
          }

          const approved = employees.map((e) => by_employee[e].approved || 0);
          const pending = employees.map((e) => by_employee[e].pending || 0);
          const declined = employees.map((e) => by_employee[e].declined || 0);
          ...
          if (employeeChart) {
            ...
            employeeChart = new Chart(ctx, { type: "bar", data, options });
          }

          const interpretEl = document.getElementById("interpret-employee");
          if (interpretEl && interpretations) {
            interpretEl.textContent = interpretations.employee;
          }
        }
        ...
        function updateDSS() {
          ...
          if (!dssContainer || !dssAlerts) return;
        ...
        function updateAnalyticsUI() {
          ...
          console.log("Updating dept head analytics UI");
          updateKPIs();
          updateStatusChart();
          updateTypeChart();
        ...
        // Initial render
        (async function init() {
          // ensure we have dept before fetching
          try {
            const r = await fetch("../api/current_user.php");
            const u = await r.json();
            if (u && u.logged_in) currentDept = (u.department || "").toString();
          } catch (e) {
        ...
        // Enhanced analytics doesn't need scope selector - it shows all data with breakdowns
      });
    </script>
  </body>
</html>
