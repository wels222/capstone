...
try {
    ...
    $oldStmt = $pdo->prepare('SELECT file_path FROM hr_signatures WHERE email = ?');
    $oldStmt->execute([$hr_email]);
    ...
    $hash = sha1($hr_email);
    $fileName = 'hr_sig_' . substr($hash, 0, 24) . '.' . $ext;
    ...
    // Save to database
    $stmt = $pdo->prepare('INSERT INTO hr_signatures (email, file_path) VALUES (?, ?) ON DUPLICATE KEY UPDATE file_path = VALUES(file_path), updated_at = CURRENT_TIMESTAMP');
    $stmt->execute([$hr_email, $relPath]);
